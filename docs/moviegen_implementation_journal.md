...
- 把 continuity 风险真正接入 `review` 分流，而不只是单独出报告
- 新增 continuity issue 索引：
  - 按 `shot_id`
  - 按 `sequence_id`
  - 按 `character_id`
- 为每个 review candidate 增加 continuity 风险策略：
  - 命中严重 continuity 类型时：
    - `decision = review`
    - `route_back_stage = review`
  - 命中 continuity issue 数达到阈值时：
    - `decision = regenerate_same_provider`
    - `route_back_stage = generate`
    - `hard_fail = true`
- 每个受影响的 candidate 现在会写入：
  - `continuity_issue_types`
  - `continuity_issue_count`
  - `decision_reasons += continuity_issue:*`
  - 必要时 `hard_fail_reasons += continuity_issue_threshold`
- 修正 review 统计顺序
  - continuity 改写后再计算 gate/payload/counts
  - 避免摘要与最终 candidate 分桶不一致
...
- 检查：
  - `workspace/review/run_20260320_234927_21968fc9__review_summary.json`
  - `workspace/review/run_20260320_234927_21968fc9__continuity_report.json`
  - `workspace/reports/run_20260320_234927_21968fc9__delivery_report.json`
  - `status --run-id run_20260320_234927_21968fc9`

### 效果

- continuity 现在已经不只是“看报告”，而是会真实改写候选去向
- 在 `run_20260320_234927_21968fc9` 中：
  - `review_candidates = 4`
  - `regenerate_candidates = 2`
  - `approved_candidates = 0`
- 原本通过的两个候选已被 continuity 风险推入 regenerate：
  - `SHOT_001` 高分 `seedance_2_0`
  - `SHOT_002` 高分 `kling_3_0`
- `gate_3_review` 的 payload 和 counts 已与 continuity 改写后的结果对齐
- `delivery_report` 也已反映 continuity 风险导致的 blocked items

### 问题

- continuity 风险模型当前仍是规则式，不是视觉级人物/服装/场景漂移检测
- 阈值和 severe 类型仍属工程启发式，后续应结合真实图像特征继续调优

### 下一步

- 如果继续推进，最自然的是把 continuity 风险继续接到 router/provider 约束上
- 或者开始补视觉级 continuity 检查

## 2026-03-20 Round 37

### 改动

- 把 continuity 风险继续往前接到 rerun 执行层
- `resume` 现在会在 continuity 触发 rerun 时，自动给目标 shot 写入 `provider_constraints`
  - `allowed_providers = [continuity_preferred_provider]`
  - `banned_providers = all_known_providers - preferred`
...
- provider 选择依据来自：
  ...
  - sequence 级 score
  - character 级 score
  - 若不足则回退到 `execution.primary_provider`
- `resume_plan.json` 现在会写出 `continuity_reroutes`
- `route` 现在真正尊重 `provider_constraints`
  - `compile-prompts` 与 `route` 已统一使用同一套 provider chain 解析
- `route_plan.selected_reason` 现在会明确标示：
  - `continuity_reroute:<provider>`
  - 或 `provider_constraints:*`
  - 不再全部伪装成普通 `route_matrix:*`

### 验证

- 修复 `resume` 中 `normalize_str_list()` 的 list 判断 bug 后重新验证
...
- 结果：
  - follow-up rerun run: `run_20260320_235622_942f0582`
- 检查：
  - `workspace/review/run_20260320_234927_21968fc9__resume_plan.json`
  - `workspace/review/run_20260320_235622_942f0582__resume_shot_specs.yaml`
  - `workspace/reports/run_20260320_235622_942f0582__route_plan.json`

### 效果

- continuity 风险现在已经能真实影响 rerun 的 provider 选择，而不是只影响 review 阶段说明
- 在本轮验证中：
  - rerun shot 为 `SHOT_001`、`SHOT_002`
  - 两个 shot 都被 continuity 策略约束到：
    - `allowed_providers = [seedance_2_0]`
  - rerun follow-up 的 `route_plan` 也已经真实只生成 `seedance_2_0` job
- `route_plan.selected_reason` 已显示：
  - `continuity_reroute:seedance_2_0`

### 问题

- continuity reroute 当前仍是规则式 provider 收敛，不是视觉级一致性驱动的 provider 选择
- 当前 gate 的 approve/reject 语义仍偏“工程执行层”，未来若要更细，还需要把 review 动作类型再拆分
...
- 把视频参考的视觉连续性检查从“单帧”升级为“多帧”
- 当前对 `video_refs` 的采样策略：
  - 首帧
  - 中间帧
  - 末帧
- pairwise 结果现在会同时输出：
  ...
  - `avg_distance`
  - `avg_color_distance`
  - `best_left_frame_index`
  - `best_right_frame_index`
  - 全量 `comparisons`
- 视觉问题判定现在使用“最大差异”而不是“最佳匹配”
  - 避免“首帧一样但后续帧分叉”被漏掉
...
- 当前仍是“少量代表帧”策略，不是稠密时序比较
- 对长视频更精细的时间连续性，后续还可继续提升
