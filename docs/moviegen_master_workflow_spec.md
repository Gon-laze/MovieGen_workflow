...
| `Seedance 2.0` | ByteDance Seed 官方中英页面可见；存在性明确 | 中文语义环境、比稿池、主力候选 | 中到强，能力可纳入路由层 | 公开页面更偏产品展示，自动化可用性需按实际接口再核验 |
| `Kling` | Kling Global 页面可见，页面显式出现 `Video Generation / Image Generation / API calls / Developer Platform` 等字样 | 常规镜头比稿、图生视频、批量尝试池 | 中到强，可接入但需按公开 API/合作方式确认 | 定价公开度不如 Vertex/Runway |
| `Vidu` | 官方中文价格页可见当前 `免费 / 标准 8 / 专业 28 / 旗舰 79` 档位；官方 `Reference to Video` 功能页明确支持 `up to 7 reference images` | 中低成本一致性镜头、参考图驱动镜头 | 强，适合作为 B/C 池候选 | 中文/英文价格页和旧 hydration 价格可能不同，落地前应以当前地区页为准 |
...
这说明 Kling 不能只被视作“网页端创作工具”，在工程设计里应被纳入 provider router 的可选目标。
...
| 主叙事镜头生产池 | `Runway Gen-4`、`Seedance 2.0`、`Kling`、`Hailuo 当前系列`、`Vidu` |
| 一致性参考图驱动镜头 | `Vidu`、`Hailuo 当前系列`、`Kling` |
...
| `Veo 3 Fast / Veo 3.1` 英雄镜头池 | `$120 - $180` |
| 中文模型信用池：`Seedance / Kling / Vidu / Hailuo` 四选二至三 | `$180 - $260` |
| `Luma` 小额试验池 | `$0 - $30` |
...
- 常规对白与叙事镜头
- 主力使用 `Runway Gen-4 / Seedance 2.0 / Kling / Hailuo / Vidu`
